					...
					<div class="row">
						...
						<div class="col-md-12"  style="display: none" id="submit-btn-field">
							<center>
								...
								<button id="sms_button" class="btn btn-success btn-sm col-sm-3" type="button"  ><i class="fas fa-sms"></i> Send SMS notification</button>
							</center>
						</div>
					</div>
				</form>
...
<script>
	var late_mobile_numbers = {};
	var absent_mobile_numbers = {};
				...
				if(resp){
					...
					if(Object.keys(data).length > 0){
						...
						Object.keys(data).map(function(k){
							...
							var id = data[k].id;
							var mobile_number = data[k].mobile_number;
							var tr = $('<tr></tr>')
							var type = !!record[k].type ? _type[record[k].type] : '';
							...
							var td = '<td>';
								...
								td += '</td>';
							tr.append(td)
							tr.append('<td>' + mobile_number + '</td>');

							if (mobile_number.length > 0) {
								if (type == 'Late')
									late_mobile_numbers[name] = mobile_number;
								else if (type == 'Absent')
									absent_mobile_numbers[name] = mobile_number;
							}
	...
	$('#sms_button').on('click', function(e){
		...
		$.ajax({
			...
			method: 'POST',
			dataType:'json',
			data:{ 'late_mobile_numbers' : JSON.stringify(late_mobile_numbers), 'absent_mobile_numbers' : JSON.stringify(absent_mobile_numbers) },
			success: function (resp) {
				console.log(resp);
				if (resp == 1) {
					alert_toast("SMS notifications has been sent successfully", 'success')
                    //setTimeout(function(){
                    //    location.reload()
                    //},1000)
				}
